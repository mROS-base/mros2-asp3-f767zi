...
void sample_lwip_scene1(void)
{
  ...
  if (err != 0) {
    syslog(LOG_ERROR, " %s %s() line=%d : scene1 connection", __FILE__, __FUNCTION__, __LINE__);
    return;
  }
  ...
  if (err != 0) {
    syslog(LOG_ERROR, " %s %s() line=%d : scene1 done", __FILE__, __FUNCTION__, __LINE__);
    return;
  }

  syslog(LOG_INFO, " %s %s() line=%d : scene1 success", __FILE__, __FUNCTION__, __LINE__);
